...
void push_token(struct Token_vector* vector, struct Token token)
{
	...
	else
	{
		...
		vector->data[vector->occupied]=token;
		vector->occupied++;
		fill_with_zeros(&vector->data[occupied], capacity/2);
	}
...
int parse(struct Char_vector* expression, struct Token_vector* tokens)
{
	int i=0;
	int token_count=0;
	int letter_count=0;
	...
	while(symbol!=0)
	{
		...
		if(type!=unknown)
		{
			if(get_token(tokens, token_count).type==unknown)
			{
...
int main()
{
	...
	struct Token_vector* tokens=create_token_vector(0);
	int count=0;
